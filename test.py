...
def main():
    ...
    page = st.sidebar.radio("Go to",
                            ["Introduction", "Data Overview", "Factor Analysis", "Country Comparison", "Conclusion"])
    ...
    if page == "Introduction":
        ...
        st.write("""
            ...
            The dataset gives insights into the factors that contribute to happiness across various countries and regions.
        ...
        """)
        st.markdown(f"You can check out the dataset and details from the Kaggle [link here.]({url})")
        ...
        st.write(f"**Note: The cleaned dataset is not applied onwards so all countries are included.**")
        ...
        st.write(
            "This displays the frequency distribution of happiness scores, showing how many countries fall into each score range.")
        ...
        fig_scatter = px.scatter(df, x=factor, y="Score", hover_data=['Country or region'],
                                 color_discrete_sequence=['#47A8BD'])
        ...
        countries = st.multiselect("Select countries to compare:", df['Country or region'].unique(),
                                   default=df['Country or region'].unique()[:3])
        ...
        if not countries:
            ...
            fig.update_layout(height=800, title_text="Comparison of Happiness Factors Across Selected Countries",
                              title_x=0.5)
